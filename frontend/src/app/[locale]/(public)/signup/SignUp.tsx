...
export function SignUp({ locale, onSuccess }: SignUpProps) {
  ...
  useEffect(() => {
    if (showEmailForm) {
      emailPasswordForm.setFocus("email");
    }
  ...
  const renderEmailPasswordStep = () => (
    <div className={styles.container}>
      ...
      <div
        className={`${styles.agreementCard} ${agreed ? styles.agreementCardChecked : ""}`}
      >
        <label className={styles.agreementLabel}>
          <span className={styles.customCheckbox}>
            <input
              ...
              aria-label={t("legal.agreeCheckbox")}
            />
            <span
              className={`${styles.checkboxVisual} ${agreed ? styles.checkboxChecked : ""}`}
              aria-hidden="true"
            >
              {agreed && <Check size={14} weight="bold" />}
            </span>
          </span>
      ...
      <div className={styles.socialButtons}>
        <button
          ...
          className={`${styles.appleButton} ${agreed ? styles.appleButtonActive : ""}`}
          aria-disabled={!agreed}
          aria-label={t("auth.startWithApple")}
        >
          ...
          {t("auth.startWithApple")}
        </button>
        {!agreed && <p className={styles.socialHint}>{t("auth.agreeFirst")}</p>}
        ...
        <button
          ...
          aria-disabled={!agreed}
          aria-label={t("auth.startWithGoogle")}
        >
      ...
      <div className={styles.loginPrompt}>
        <span className={styles.loginPromptText}>
          ...
          <Link href={`/${resolvedLocale}/login`} className={styles.loginLink}>
            {t("auth.login")}
          </Link>
        </span>
      </div>
