...
export function TagManagementPageClient({
	...
	const tagListRefs = useRef<Record<CategoryKey, HTMLDivElement | null>>({
		tori: null,
		uke: null,
		waza: null,
	});
	...
	const handleTouchMove = (
		...
		const tagElement = element?.closest<HTMLDivElement>("[data-tag-id]");
		...
		const listElement = element?.closest<HTMLDivElement>("[data-tag-list]");
		...
		if (activeTouch.clientY < rect.top) {
			const firstTag =
				registeredList.querySelector<HTMLDivElement>("[data-tag-id]");
