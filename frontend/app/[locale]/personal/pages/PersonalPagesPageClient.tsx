...
export function PersonalPagesPageClient() {
			...
			try {
				...
				while (hasMoreData) {
					...
					if (response.success && response.data) {
						...
						throw new Error(response.error || t("personalPages.dataFetchFailed"));
					}
				}
	...
	const handleSavePage = async (pageData: PageCreateData) => {
		try {
			...
			alert(
				error instanceof Error ? error.message : t("personalPages.pageCreationFailed"),
			);
		}
	};
	...
	const handleUpdatePage = async (pageData: UpdatePagePayload) => {
		try {
			...
			alert(
				error instanceof Error ? error.message : t("personalPages.pageUpdateFailed"),
			);
		}
	};
				...
				<div className={styles.pageListDescription}>
					...
					<p className={styles.pageCount} data-testid="page-count">
						{allTrainingPageData.length === displayedTrainingPageData.length
							? t("personalPages.showingAll", { total: allTrainingPageData.length })
