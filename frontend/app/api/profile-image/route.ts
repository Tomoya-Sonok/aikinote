...
import { generatePublicUrl, extractFileKeyFromUrl, deleteFileFromS3 } from '@/lib/aws-s3'
import { z } from 'zod'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (currentProfile?.profile_image_url) {
      try {
        const oldFileKey = extractFileKeyFromUrl(currentProfile.profile_image_url)
