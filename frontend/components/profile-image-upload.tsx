...
export function ProfileImageUpload({
  ...
  const getUploadUrl = async (file: File): Promise<UploadResponse> => {
    ...
    console.log('アップロードURL API レスポンス:', response.status, response.statusText)
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.addEventListener('load', () => {
        if (xhr.status === 200) {
          ...
          reject(new Error(`アップロードが失敗しました。ステータス: ${xhr.status}`))
        }
      })
  ...
  const updateProfileImage = async (fileKey: string): Promise<{ imageUrl: string }> => {
