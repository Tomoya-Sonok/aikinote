...
app.get("/:userId", async (c) => {
	try {
		...
		// JWT認証
		console.log("GET認証ヘッダー:", authHeader ? "存在" : "なし");
		const token = extractTokenFromHeader(authHeader);
		console.log("GET抽出したトークン長:", token.length);
		const payload = verifyToken(token);
		console.log("GETJWT検証成功:", { userId: payload.userId });
...
app.put("/:userId", zValidator("json", updateProfileSchema), async (c) => {
	try {
		...
		// JWT認証
		console.log("PUT認証ヘッダー:", authHeader ? "存在" : "なし");
		const token = extractTokenFromHeader(authHeader);
		console.log("PUT抽出したトークン長:", token.length);
		const payload = verifyToken(token);
		console.log("PUTJWT検証成功:", { userId: payload.userId });
