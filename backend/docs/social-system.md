...
| | 7日以内の投稿 | +10 |
| | それ以外 | 0 |
| 新規ユーザーブースト | 登録14日以内のユーザーの投稿 | +30 |
| | 登録30日以内のユーザーの投稿 | +15 |
| | それ以外 | 0 |
| ランダムジッター | 全投稿に一律適用 | +0〜15（ランダム） |

**設計意図:**
- 鮮度ボーナスを4段階に細分化し、72時間超〜7日以内にも軽いブーストを付与。投稿頻度が低い初期フェーズでもフィードが活きて見える
- 新規ユーザーブーストにより、新規登録者の投稿がフィードで発見されやすくなり、初投稿が埋もれることによる離脱を防止
- ランダムジッター（0〜15）により、同程度のスコアの投稿の表示順がリロードごとに変化。AikiNoteの「非競争的」思想と整合し、全ての投稿に発見の機会を提供
...
| フィードソート | PostgreSQL関数 | SQL側でスコア計算+LIMIT/OFFSETが効率的。鮮度ボーナス4段階+新規ユーザーブースト+ランダムジッターでユーザー獲得フェーズに最適化 |
